...
exports.family_events_endpoint = (req, res) => {
    ...
    db.getAllEvents().then((list) => {
        ...
        console.log('Error getting family events:', err);
        res.status(500).json({ error: 'Error retrieving events' });
    });
}
...
exports.new_event_entry = (req, res) => {
    userDAO.getAllUsersInFamily(req.family).then((users) => {
        ...
        console.log('Error getting users in family:', err);
...
exports.show_edit_event = (req, res) => {
    ...
    const currentUserFamily = req.user.familyId;
    db.getEventById(eventId).then((event) => {
        ...
        if (event.familyId !== currentUserFamily) {
            return res.status(403).send('Forbidden: Different Family');
        }

        if (event.organiser !== currentUser && req.user.role !== 'administrator') {
            return res.status(403).send('Forbidden: Not Organiser');
        }
...
exports.update_event = (req, res) => {
    ...
    db.getEventById(eventId).then((event) => {
        ...
        if (event.organiser !== currentUser && req.user.role !== 'administrator') {
            return res.status(403).json({ success: false, msg: 'Forbidden: Not Organiser' });
        }
        ...
        db.updateEvent(eventId, updateData).then((numUpdated) => {
            ...
            .catch((err) => {
                ...
                res.status(500).json({ success: false, msg: 'Error updating event' });
            });
    })
        .catch((err) => {
            console.log('Error fetching event:', err);
            res.status(500).json({ success: false, msg: 'Error retrieving event' });
        });
};
...
exports.delete_event = (req, res) => {
    ...
    console.log(`Deleting event ${eventId} by ${currentUser}`);
    ...
    db.getEventById(eventId).then((event) => {
        ...
        if (event.organiser !== currentUser && req.user.role !== 'administrator') {
            return res.status(403).json({ 'message': 'Forbidden: Not Organiser' });
        }
